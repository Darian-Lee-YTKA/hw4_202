...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, batch_size, char_vocab_dim):
        ...
        self.lstm = nn.LSTM(embedding_dim, hidden_dim // 2,
                            ...
                            dropout=.4)  # normal bilstm with batch false
        ...
        self.transitions = nn.Parameter(
            # nn.parameters means that these parameters will be updated through gradient descent
    ...
    def init_hidden(self):
        return (torch.randn(2, self.batch_size, self.hidden_dim // 2),
                torch.randn(2, self.batch_size,
                            self.hidden_dim // 2))  # intialize for random weights. Independent of batching
    ...
    def _forward_alg(self, feats, get_all_scores=False):
        ...
        for sequence_token in range(seq_len):
            ...
            for next_tag in range(self.tagset_size):
                ...
                # print(feat[:, next_tag])  #
                emit_score = feat[:, next_tag].unsqueeze(1).expand(batch_size,
                                                                   self.tagset_size)  # broadcasts the scalar emit score to be the size of the tag_set
                ...
                if alphas_t is None:
                    ...
                    alphas_t = torch.cat((alphas_t, log_sum_next_var),
                                         dim=1)  # this is the total sum of getting to the next point
    ...
    def _score_sentence(self, feats, tags):
        ...
        for i in range(seq_len):
            ...
            score = score + self.transitions[tags[:, i + 1], tags[:, i]] + feat.gather(1, tags[:, i + 1].unsqueeze(
                1)).squeeze(1)
        ...
        for seq_token in range(seq_len):
            ...
            for next_tag in range(self.tagset_size):  # this is the tag that we are transitioning too
                ...
                best_tag_id = torch.argmax(next_tag_var,
                                           dim=-1)  # we are only saving the best path (through the best past tag) to get to the current tag
                ...
                if bptrs_t is None:
                    ...
                    bptrs_t = torch.cat((bptrs_t, best_tag_id.unsqueeze(1)),
                                        dim=-1)  # # saves this information. We do this because we want to follow the trail of backpointers for the one that ends in stop
                ...
                if viterbivars_t is None:
                    ...
                    viterbivars_t = torch.cat((viterbivars_t, next_tag_var.gather(1, best_tag_id.unsqueeze(1))),
                                              dim=-1)  # gets the max by indexing at the max index. Then tranposes to be of shape(1)
    ...
    def _svm_loss(self, sentence, chars, tags):
        ...
        # Compute the gold score for the true tag sequence
        gold_score = self._score_sentence(feats, tags)  # [batch_size]

        # Compute terminal scores from forward algorithm
        terminal_scores = self._forward_alg(feats, get_all_scores=True)  # [batch_size, num_tags]

        # Compute Hamming loss as structured margin cost (ignoring padding)
        hamming_loss = self._get_hamming_score(feats, tags)

        # Compute the max over all possible sequences
        max_margin_violation = (terminal_scores + hamming_loss).max(dim=1)[0]  # [batch_size]

        # Compute the structured SVM loss
        loss = max_margin_violation - gold_score  # Matches equation form

        # Apply mask to ignore padding loss (where the true tags are 0)
        loss = loss * mask.any(dim=1).float()  # Only keep nonzero losses per sequence
...
class Character_CNN(nn.Module):
    ...
    def forward(self, char_inputs):
        ...
        conv_out = self.conv1d(char_embeds.reshape(batch_size * word_seq_len, 64,
                                                   char_seq_len))  # (batch_size * word_seq_len, 32, new_char_seq_len)
    ...
    for seq in char_inputs:
        ...
        for word in seq:
            ...
            if word.shape[0] < max_char_len_per_word:
                padding_size = max_char_len_per_word - word.shape[
                    0]  # the difference between itself and the max char len
...
model = BiLSTM_CRF(len(train_dataset.vocab), train_dataset.tag_vocab, EMBEDDING_DIM, HIDDEN_DIM, 8,
                   char_vocab_dim=len(train_dataset.char_vocab))
...
folder = os.path.join(base_dir, "normal_loss_cnn")
    ...
    with torch.no_grad():
        for batch in dev_loader:
            ...
            if sentence.shape[0] != 8:
                ...
                loss = model.neg_log_likelihood(sentence, chars, tags)
                model.add_batch_predictions_to_export_file(sentence, chars, val_file_true)
